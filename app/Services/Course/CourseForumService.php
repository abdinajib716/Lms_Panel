...
class CourseForumService extends MediaService
{
   ...
   {
      return CourseForum::with(['user', 'course', 'sectionLesson', 'replies.user'])
         ->where('id', $id)
         ->first();
   }

   function getForumsByCourse(string $courseId): Collection
   {
      return CourseForum::with(['user', 'sectionLesson', 'replies'])
         ->where('course_id', $courseId)
         ->orderBy('created_at', 'desc')
         ->get();
   }

   function getForumsByLesson(string $lessonId): Collection
   {
      return CourseForum::with(['user', 'replies.user'])
         ->where('section_lesson_id', $lessonId)
         ->orderBy('created_at', 'desc')
         ->get();
   }

   function getUserForums(string $userId): Collection
   {
      return CourseForum::with(['course', 'replies'])
         ->where('user_id', $userId)
         ->orderBy('created_at', 'desc')
         ->get();
   }

   function createReply(array $data): CourseForumReply
   {
      return CourseForumReply::create($data);
   }
