...
use App\Http\Controllers\WaafiPayWebController;
...
// WaafiPay payment routes
Route::prefix('waafipay')->controller(WaafiPayWebController::class)->group(function () {
    Route::get('pay/{courseId}', 'show')->name('waafipay.show');
    Route::post('initiate', 'initiate')->name('waafipay.initiate');
    Route::post('check-status', 'checkStatus')->name('waafipay.check-status');
});

// Student transaction history
Route::get('my-transactions', [WaafiPayWebController::class, 'transactions'])->name('student.transactions');
