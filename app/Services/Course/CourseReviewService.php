...
class CourseReviewService extends MediaService
{
   ...
   function getUserReview(string $userId, string $courseId): CourseReview | null
   {
      return CourseReview::where('user_id', $userId)->where('course_id', $courseId)->first();
   }

   function getUserReviews(string $userId): Collection
   {
      return CourseReview::with(['course'])->where('user_id', $userId)->orderBy('created_at', 'desc')->get();
   }

   function getReviewById(string $id): CourseReview | null
   {
      return CourseReview::find($id);
   }

   function getCourseReviewStats(string $courseId): array
   {
      $stats = $this->totalReviews($courseId);
      $avgRating = CourseReview::where('course_id', $courseId)->avg('rating') ?? 0;
